exports.run = (client, message, args) => {
  const fs = require('fs')
  if (message.channel.id !== '533553472453476362' && message.channel.id !== '535372316901769217') {
    message.channel.send(`Jesteś na złym kanale. Prawidłowy kanał to ${client.channels.get('533553472453476362')}!`)
    return
  }
  if (!args[0]) {
  ...
  if (args[0] === 'help') {
    ...
    let sample = fs.readdirSync('./audio/')
  ...
  var name
  var isPresent = false
  let sampel = args[0].toLowerCase()

  function check (file) {
    let plik = file.slice(0, -4)
    if (sampel === plik) {
      isPresent = true
    }
  }
